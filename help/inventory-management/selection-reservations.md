---
title: Algoritmi di origine e prenotazioni
description: Scopri l’Algoritmo di selezione origine e i sistemi di prenotazioni eseguiti in background per mantenere aggiornate le quantità vendibili.
...
# Algoritmi di origine e prenotazioni

Il cuore di [!DNL Inventory Management] tiene traccia di tutti i prodotti disponibili, sia virtualmente che a portata di mano, nei magazzini e nei negozi. I sistemi Algoritmo di selezione origine e Impegni vengono eseguiti in background, mantenendo aggiornate le quantità vendibili, evitando collisioni e consigliando opzioni di spedizione.

>[!NOTE]
>
>Consulta la sezione [documentazione per sviluppatori](https://developer.adobe.com/commerce/php/development/framework/inventory-management/) per informazioni sull&#39;utilizzo di [!DNL Inventory Management] a livello di programmazione.

## Algoritmo di selezione della sorgente

L&#39;algoritmo di selezione origine (SSA) analizza e determina la corrispondenza migliore per le origini e la spedizione utilizzando l&#39;ordine di priorità delle origini configurate in un magazzino. Durante la spedizione dell&#39;ordine, l&#39;algoritmo fornisce un elenco consigliato di origini, quantità disponibili e importi da dedurre in base all&#39;algoritmo selezionato. [!DNL Inventory Management] fornisce un algoritmo Priority e supporta le estensioni per le nuove opzioni.
...
**Registra inventario** - Utilizzando le scorte e le fonti, l&#39;SSA controlla il canale di vendita delle richieste di prodotti in entrata e determina le scorte disponibili:
...
**Gestisci spedizioni** - L&#39;algoritmo facilita l&#39;elaborazione e la spedizione degli ordini. Puoi eseguire l’algoritmo per ottenere consigli sulle origini migliori per la spedizione del prodotto o per ignorare le selezioni su:
...
Per configurare, assegnare e ordinare origini a un magazzino personalizzato. Consulta [Assegnazione di priorità alle origini per un Stock](stocks-prioritize-sources.md).
...
- **GOOGLE MAP** - Utilizzi [Piattaforma Google Maps][1] servizi per calcolare la distanza e l&#39;ora tra l&#39;indirizzo della destinazione di spedizione e le posizioni di origine (indirizzo e coordinate GPS). Questa opzione utilizza i valori di latitudine e longitudine della sorgente. È necessaria una chiave API Google con [API di geocodifica][2] e [API della matrice di distanza][3] abilitato. Questa opzione richiede un piano di fatturazione Google e può comportare costi attraverso Google.
...
Per configurare, seleziona le configurazioni e completa i passaggi aggiuntivi, ad esempio la chiave API di Google o il download dei dati di spedizione. Consulta [Configurare l&#39;algoritmo di priorità della distanza](distance-priority-algorithm.md).
...
>[!NOTE]
>
>La funzionalità di prenotazione richiede `inventory.reservations.updateSalabilityStatus` il consumer della coda messaggi deve essere eseguito in modo continuo. Per verificare se è in esecuzione, utilizza `bin/magento queue:consumers:list` comando. Se il consumer della coda messaggi non è elencato, avvialo: `bin/magento queue:consumers:start inventory.reservations.updateSalabilityStatus`.
...
![Prenotazioni da ordine a consegna](assets/diagram-quantity.png){width="600" zoomable="yes"}

Un cliente invia un ordine. [!DNL Commerce] controlla la quantità di magazzino venduta corrente. Se è disponibile una quantità sufficiente di scorte a livello di scorte, un impegno inserisce un blocco temporaneo per lo SKU del prodotto (per tale scorte) e ricalcola la quantità vendibile.

Dopo aver fatturato l&#39;ordine, è possibile determinare gli importi dei prodotti da detrarre e spedire dalle origini. La spedizione viene elaborata e inviata al cliente da una o più origini selezionate. Le quantità vengono detratte automaticamente dalla quantità di magazzino di origine e gli impegni vengono cancellati. Per informazioni ed esempi completi, consulta [Informazioni sullo stato degli ordini e sugli impegni](order-status.md).
...
Al termine dell&#39;elaborazione di un ordine (Completo, Annullato, Chiuso), tutti gli impegni nell&#39;ambito dell&#39;ordine devono essere risolti in `0`. In questo modo vengono cancellati tutti i blocchi di quantità vendibili.

>[!NOTE]
>
>Anche gli ordini inevasi (con soglie esaurite) e le impostazioni Notifica per quantità inferiore alla soglia influiscono sul calcolo delle quantità vendibili, ma non rientrano nell&#39;ambito di questo argomento. Per ulteriori informazioni su queste impostazioni, consulta [Configurazione [!DNL Inventory Management]](./configuration.md).
...
Metadati `event_type` può avere i seguenti valori:
...
1. Il cliente effettua un ordine di acquisto per 25 unità di prodotto `SKU-1`. La prenotazione contiene le seguenti informazioni:
...
I tre `quantity` somma dei valori fino a 0 (-25 + 5 + 20). Il sistema non modifica le prenotazioni esistenti.

## Rimozione delle prenotazioni elaborate

Il `inventory_cleanup_reservations` Il processo cron esegue query SQL per cancellare la tabella del database delle prenotazioni. Per impostazione predefinita viene eseguito ogni giorno a mezzanotte, ma puoi configurare l’ora e la frequenza. Il processo cron esegue uno script che esegue query sul database per trovare sequenze di prenotazione complete in cui la somma dei valori di quantità è 0. Quando tutte le prenotazioni per un determinato prodotto che hanno avuto origine nello stesso giorno (o in un’altra ora configurata) sono state compensate, il processo cron elimina tutte le prenotazioni contemporaneamente.

Il `inventory_reservations_cleanup` il processo cron non è uguale al processo `inventory.reservations.cleanup` consumer coda messaggi. Il consumatore elimina in modo asincrono le prenotazioni per SKU prodotto dopo la rimozione di un prodotto, mentre il processo cron cancella l’intera tabella delle prenotazioni. Il consumatore è richiesto quando si abilita [**Sincronizza con catalogo**](../configuration-reference/catalog/inventory.md) opzione di magazzino nella configurazione archivio. Consulta [Gestire le code dei messaggi](https://experienceleague.adobe.com/docs/commerce-operations/configuration-guide/message-queues/manage-message-queues.html) nel _Guida alla configurazione_.
...
>[!NOTE]
>
>Esistono comandi CLI che è possibile utilizzare per rilevare e gestire le incoerenze nelle prenotazioni (vedere [[!DNL Inventory Management] Riferimento CLI](cli.md)).

### Aggiornamenti prenotazione

Quando le modifiche vengono completate negli ordini e negli importi dei prodotti, [!DNL Commerce] inserisce automaticamente le compensazioni per la prenotazione. Non è necessario immettere compensazioni tramite l&#39;amministratore o il codice per aggiornare o cancellare questi blocchi. Gli impegni sono interessati solo dagli impegni inseriti per bloccare una quantità o per cancellare un importo di blocco (compensando gli impegni).

Ecco come funzionano:

- **Ordine inviato** - Quando un ordine viene inviato per diversi prodotti, una prenotazione inserisce per tale importo. Ad esempio, ordinando cinque zaini da un sito web statunitense si immette una prenotazione di `-5` per la SKU e le scorte. La quantità vendibile viene ridotta di 5.

- **Ordine annullato** - Quando un ordine viene annullato (in tutto o in parte), viene inserito un impegno di retribuzione per liquidare tale importo. Ad esempio, l’annullamento di tre zaini comporta l’inserimento di una prenotazione +3 per lo SKU e il magazzino in questione, cancellando il blocco. La quantità vendibile viene aumentata di 3.

- **Ordine spedito** - Quando un ordine viene spedito (in tutto o in parte), una prenotazione di compensazione entra per liquidare tale importo. Ad esempio, la spedizione di due zaini immette una prenotazione +2 per lo SKU e il magazzino, cancellando il blocco. La quantità di prodotto viene direttamente ridotta di 2 per la spedizione. Anche la quantità vendibile calcolata viene aggiornata per l’importo delle scorte ridotto, ma non è più interessata dalla prenotazione.
