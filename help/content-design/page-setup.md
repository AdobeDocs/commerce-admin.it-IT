...
![Sezioni di pagina di HTML](./assets/storefront-home-html-inspect.png){width="700" zoomable="yes"}

## Testa HTML

Le impostazioni nella sezione Testa HTML corrispondono a `<head>` di una pagina HTML e può essere configurato per ogni visualizzazione store. Oltre ai metadati per il titolo della pagina, la descrizione e le parole chiave, la sezione include un collegamento al favicon e script vari. In questa sezione sono anche configurate le istruzioni per i robot dei motori di ricerca e la visualizzazione dell’avviso di dimostrazione del negozio.
...
1. Sotto _Altre impostazioni_, espandi ![Selettore di espansione](../assets/icon-display-expand.png) il **[!UICONTROL HTML Head]** sezione.

   ![Impostazioni di configurazione HTML Head](./assets/configuration-html-head.png){width="500" zoomable="yes"}

1. Aggiornare il [favicon](../getting-started/storefront-branding.md#add-a-favicon) se necessario.
...
1. Inserisci qualsiasi **[!UICONTROL Scripts and Style Sheets]** secondo necessità.

1. Attiva o disattiva la [avviso negozio demo](../getting-started/storefront-branding.md#set-the-store-demo-notice) se necessario.
...
| [!UICONTROL Scripts and Style Sheets] | Visualizzazione store | Contiene gli script che devono essere inclusi nel HTML prima della chiusura `<head>` tag. Ad esempio, qualsiasi JavaScript di terze parti che deve essere inserito prima del `<body>` Il tag può essere inserito qui. |
...
![Impostazioni di configurazione dell’intestazione](./assets/configuration-header.png){width="400" zoomable="yes"}
...
1. Sotto _Altre impostazioni_, espandi ![Selettore di espansione](../assets/icon-display-expand.png) il **[!UICONTROL Header]** sezione.

1. Apporta le modifiche necessarie per la visualizzazione dello store:

   - [Logo](../getting-started/storefront-branding.md#upload-your-logo) impostazioni
   - [Messaggio di benvenuto](../getting-started/storefront-branding.md#change-the-welcome-message) impostazioni
...
| [!UICONTROL Translate Title] | Visualizzazione store | Determina se `Page Title` o `Meta Title` Dovrebbe essere tradotto. |

{style="table-layout:auto"}

## Piè di pagina

La sezione di configurazione del piè di pagina è il punto in cui puoi aggiornare [avviso di copyright](../getting-started/storefront-branding.md#change-the-copyright-notice) nella parte inferiore della pagina e immettere script vari che devono essere posizionati prima della chiusura `<body>` tag.

![Impostazioni di configurazione piè di pagina](./assets/configuration-footer.png){width="400" zoomable="yes"}
...
1. Apporta le modifiche necessarie al **[!UICONTROL Copyright]** e **[!UICONTROL Miscellaneous HTML]** impostazioni.
...
| Campo | Ambito | Descrizione |
|--- |--- |--- |
| [!UICONTROL Miscellaneous HTML] | Visualizzazione store | Una casella di input in cui è possibile caricare script vari sul server che devono essere inseriti immediatamente prima della chiusura `<body>` tag. |
| [!UICONTROL Copyright] | Visualizzazione store | Dichiarazione di copyright visualizzata nella parte inferiore di ogni pagina. Per includere il simbolo di copyright, utilizzare l&#39;entità carattere HTML `\&copy;` come segue: `\&copy; 2021 Commerce Demo Store. All Rights Reserved.` Assicurati di sostituire l’avviso di copyright di esempio con il tuo. |
