...
description: Scopri come Adobe Commerce calcola l’imposta sul valore aggiunto (IVA) utilizzando le regole relative all’indirizzo di un esercente o di un cliente e come applicare diverse aliquote IVA per prodotto o servizio.
...
| Regola fiscale #1 |  |
|--- |--- |
| Classe imposta cliente | Le classi di imposta cliente devono includere: <br />Una classe per i clienti nazionali. <br />Classe per i clienti con ID IVA formattati in modo errato.<br />Classe per i clienti la cui convalida dell&#39;ID IVA non è riuscita. |
