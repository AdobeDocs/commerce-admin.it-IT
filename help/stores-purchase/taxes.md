...
Configura il tuo store per calcolare le tasse secondo i requisiti della tua lingua. È possibile impostare [classi fiscali](tax-class.md) per prodotti e gruppi di clienti e creare [norme fiscali](tax-rules.md) che combinano classi di prodotti e clienti, aree fiscali e aliquote. Commerce fornisce anche impostazioni di configurazione per imposte sui prodotti fisse, imposte composte e visualizzazione dei prezzi oltre i confini internazionali. Se è necessario raccogliere un [imposta sul valore aggiunto](vat.md), è possibile impostare lo store per calcolare automaticamente la quantità appropriata con la convalida.

>[!NOTE]
>
>Le versioni da 2.4.0 a 2.4.3 di Adobe Commerce e Magento Open Source includevano l’estensione sviluppata dal fornitore Vertex utilizzata per l’integrazione con Vertex Cloud al fine di fornire gestione fiscale e pulizia degli indirizzi. A partire dalla versione 2.4.4, questa estensione non è più inclusa nella versione di base e deve essere installata e aggiornata dalla versione di Commerce Marketplace o direttamente dal fornitore. [Vertice contatto](https://marketplace.magento.com/partner/vertex_inc) per informazioni sull’estensione e sulla documentazione.<br><br>
>
>Se l’estensione in bundle è abilitata e configurata, devi aggiornare il file compositore.json come parte del processo di aggiornamento 2.4.4 e gestire gli aggiornamenti delle estensioni in futuro. Consulta [Moduli di aggiornamento](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/modules/upgrade.html) nel _Guida all’aggiornamento_.

## Riferimento rapido

Alcune impostazioni di imposta dispongono di una scelta di opzioni che determinano il modo in cui l&#39;imposta viene calcolata e presentata al cliente. Per ulteriori informazioni, consulta [Linee guida internazionali in materia fiscale](international-tax-guidelines.md).
...
|--- |--- |
| [!UICONTROL Unit Price] | Commerce calcola l’imposta per ciascun articolo e visualizza i prezzi comprensivi di imposta. Per calcolare il totale dell&#39;imposta, viene arrotondata l&#39;imposta per ogni elemento e quindi viene sommata. |
| [!UICONTROL Row Total] | Commerce calcola l’imposta per ogni linea. Per calcolare il totale dell&#39;imposta, viene arrotondata l&#39;imposta per ogni voce di riga e quindi viene sommata. |
...
I possibili campi di visualizzazione variano a seconda del metodo di calcolo e del fatto che i prezzi del catalogo includano o escludano le imposte. I campi di visualizzazione hanno precisione a due decimali nei calcoli normali. Alcune combinazioni di impostazioni di prezzo visualizzano i prezzi che includono ed escludono l&#39;imposta. Quando entrambi vengono visualizzati sullo stesso elemento di riga, i clienti possono confonderli e viene attivata una [avvertenza](taxes.md#warning-messages).
...
>[!IMPORTANT]
>
>Vi sono modifiche rispetto alle versioni precedenti per gli esercenti dell&#39;UE o altri commercianti IVA che espongono i prezzi, comprese le imposte, e operano in diversi paesi con più punti di vendita. Se carichi i prezzi con più di due cifre di precisione, Commerce arrotonda automaticamente tutti i prezzi a due cifre per garantire che venga presentato un prezzo coerente agli acquirenti.
...
1. Crea [norme fiscali](tax-rules.md) per i beni fisici, utilizzando le classi fiscali dei prodotti fisici e associarli alle aliquote fiscali appropriate.
...
- [Commissione europea Fiscalità e unione doganale][1]
- [EU 1015 Cambiamenti del luogo di fornitura][2]
